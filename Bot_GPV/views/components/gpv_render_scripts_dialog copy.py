...
class ScriptDialog:
    ...
    def render_ai_config_panel(ctrl, p, s, mod_name, form_name, ai_script):
        ...
        with st.expander("📄 Tùy chỉnh Workflow & Mục tiêu", expanded=False):
            # Giữ lại scenarios để AI biết đang làm ADD, EDIT hay DELETE
            scenarios = st.multiselect("Mục tiêu:", options=["ADD", "EDIT", "DELETE", "WORKFLOW"], default=["ADD"], key=f"sc_{s['id']}")
            
            # Khởi tạo workflow_ctx chỉ dành cho Nối cảnh (nếu có)
            workflow_ctx = ""
            if "WORKFLOW" in scenarios:
                ...
                for f in selected_next:
                    # Lấy metadata của các cảnh sau nhét vào
                    f_meta = ctrl.get_formatted_meta_for_ai(f['id'])
                    workflow_ctx += f"\n\n-- TIẾP THEO: {f.get('sub_title')} --\n{json.dumps(f_meta, ensure_ascii=False)}"

        # --- BƯỚC 3.1: NHÚNG BLUEPRINT CHÍNH (THAY THẾ HOÀN TOÀN NGHIỆP VỤ CŨ) ---
        ai_blueprint = ctrl.get_formatted_meta_for_ai(s['id'])
        blueprint_json_str = json.dumps(ai_blueprint, indent=2, ensure_ascii=False)
        
        # Tạo khối dữ liệu sạch nhất gửi AI
        main_blueprint_ctx = f"""
--- 📚 HỆ THỐNG BLUEPRINT (DỮ LIỆU THỰC TẾ) ---
{blueprint_json_str}

--- 🛠 QUY TẮC TRÍCH XUẤT ---
1. Dựa vào 'execution_flow' để xác định trình tự hành động.
2. Với mỗi bước, lấy 'selector' chính xác từ 'interface_specs'.
3. Tự tạo 'value' nhập liệu thực tế dựa trên nhãn (label) của field.
"""
        ...
        # 4. Tạo nội dung bức thư thô (Bỏ user_edited_ctx dư thừa)
        # Chỉ gửi: Blueprint chính + Luồng nối cảnh (nếu có)
        full_draft_prompt = ai_script.generate_ai_prompt(
            mod_name, 
            form_name, 
            temp_config, 
            main_blueprint_ctx + workflow_ctx
        )

        # 5. Khu vực Kiểm duyệt
        st.write("### 🎬 Kiểm duyệt nội dung")
        with st.expander("🔍 KIỂM TRA BỨC THƯ GỬI AI", expanded=False):
            final_prompt = st.text_area(
                "Nội dung chuẩn bị gửi đi:", 
                value=full_draft_prompt, 
                height=450, 
                key=f"f_p_{s['id']}"
            )
